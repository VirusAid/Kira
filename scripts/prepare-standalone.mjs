...
// Офлайн-мозг (Ollama + модель Qwen3) НЕ вшивается в установщик: он остаётся
// лёгким (~450 МБ, влезает в лимит релиза GitHub 2 ГБ), а движок и модель Kira
// докачивает при первом запуске в записываемую папку данных пользователя
// (см. src/main/modules/ai/localLlm.ts → setupBrain). Так «из коробки»
// превращается в «за одну автозагрузку», зато установщик хостится на GitHub.
...
async function main() {
  ...
  console.log('   офлайн-мозг Kira докачает при первом запуске).')
}
