...
const STEPS: { title: string; subtitle: string; fields: Field[]; ai?: boolean }[] = [
  {
    ...
    fields: [
      // намеренно БЕЗ примера с конкретным именем: имя разработчика в подсказке
      // сбивало пользователей (и раньше было зашито в дефолт → «Привет, Вадим»)
      { key: 'name', label: 'Твоё имя', placeholder: 'Как к тебе обращаться?' }
    ]
  },
  {
