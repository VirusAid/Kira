 ...
 *
 * ПОРЯДОК В ФАЙЛЕ — для читаемости, а не для поведения. Приоритет сопоставления
 * задаётся полем priority: универсальные «ловушки» (например, «открой что
 * угодно») помечены отрицательным значением и проверяются последними. Раньше
 * приоритет определялся местом в массиве, и перенос действия менял поведение
 * молча.
 *
...
import {
  ...
  NoteController, PowerController, ReminderController, SearchController, SnippetController,
  SystemController, UtilityController, WindowController, knownFolder
} from '../controllers'
...
/**
 * Расширения файлов — одним списком на весь модуль.
 *
 * Тут решается давняя путаница: «report.pdf» и «youtube.com» по форме
 * одинаковы — слово, точка, слово. Отличить их можно только по словарю, и
 * словарь должен быть ОДИН: пока «открыть сайт» и «открыть файл» судили каждый
 * по-своему, «открой report.pdf» уезжало в браузер, а «открой youtube.com»
 * рисковало уйти в проводник. Русские имена под шаблон домена не подходили
 * никогда — поэтому ломалось только на английских, что и выглядело как
 * «не открывает файлы на английском».
 */
const FILE_EXT = 'pdf|txt|docx?|xlsx?|pptx?|rtf|odt|epub|exe|msi|bat|cmd|ps1|sh|py|js|ts|jsx|tsx|json|xml|ya?ml|csv|md|log|ini|cfg|conf|zip|rar|7z|iso|dll|lnk|apk|torrent|png|jpe?g|gif|webp|bmp|svg|ico|mp3|wav|flac|ogg|mp4|avi|mkv|mov|webm'
/** Глаголы открытия — по-русски и по-английски: имена программ и так латиницей. */
const OPEN_VERBS = 'открой|открыть|запусти|запустить|включи|open|launch|run|start'
...
export const actions: KiraAction[] = [
  ...
  },
  {
    ...
    patterns: [
      // Расширения файлов исключены НАМЕРЕННО — см. FILE_EXT выше.
      new RegExp(`^(?:открой|зайди на|перейди на|open|go to)\\s+(?:сайт\\s+)?(?<url>[a-z0-9а-я][a-z0-9а-я.-]*\\.(?!(?:${FILE_EXT})$)[a-zа-я]{2,}(?:\\/\\S*)?)$`),
      /^(?:открой|зайди на|перейди на|open|go to)\s+сайт\s+(?<url>\S+)$/
    ],
  ...
  },
  {
    ...
    patterns: [/^(?:следующ(?:ий|ая|ую)(?:\s+(?:трек|песн(?:я|ю)|композици(?:я|ю)))?|переключи(?:\s+(?:трек|песню))?|дальше|некст)$/],
  ...
  },
  {
    ...
    patterns: [/^(?:по)?тише$|^(?:сделай|говори)\s+(?:по)?тише$|^(?:убавь|уменьши|приглуши)(?:\s+(?:звук|громкость))?$/],
    examples: ['тише'],
  ...
  },
  {
    ...
    patterns: [
      // «поставь громкость на 30» — обычная, а не какая-то особенная
      // формулировка; без ведущего глагола она уезжала в облако
      /^(?:поставь|сделай|установи|выстави|ставь)?\s*(?:громкость|звук)\s+(?:на\s+)?(?<level>\d{1,3})(?:\s*%|\s+процентов)?$/,
      /^(?:поставь|сделай|установи|выстави|ставь)?\s*(?:громкость|звук)\s+на\s+(?<preset>максимум|полную|всю|минимум)$/
    ],
  ...
  // ─── Система ──────────────────────────────────────────────────────────────
  {
    ...
    patterns: [/^(?:сделай|сними|сохрани)?\s*(?:скриншот|скрин|снимок экрана|снимок с экрана)$|^(?:сними|сфоткай|заскринь)\s+экран$/],
...

  {
    ...
    patterns: [
      ...
      /^(?:покажи|скажи)\s+(?:что|какие программы)\s+(?:сейчас\s+)?(?:запущено|запущены|работает|работают|открыто|открыты)$/,
  ...
  // ─── Питание (опасные) ────────────────────────────────────────────────────
  {
    ...
    description: 'Принудительно закрывает зависшую программу по имени',
    category: 'system',
    aliases: ['убей процесс', 'заверши процесс'],
    patterns: [/^(?:убей|заверши|прибей|сними|закрой)\s+(?:процесс|задачу|зависш(?:ий|ую))\s+(?<name>[\wа-я.+-]+)$/],
    examples: ['убей процесс chrome'],
    phrases: ['заверши зависшую программу', 'сними задачу'],
    args: [{ name: 'name', description: 'Имя программы или её номер', required: true }],
    // принудительное завершение теряет несохранённое — спрашиваем
    dangerous: true,
    describe: (a) => `Принудительно завершить «${a.name}»`,
    execute: (a) => SystemController.killProcess(a.name ?? ''),
    confirmText: (a) => `Завершила ${a.name}`
  },
  {
    id: 'lock_screen',
    ...
    patterns: [/^(?:заблокируй|залочь|запри)(?:\s+(?:компьютер|комп|экран|пк|систему|ноутбук))?$/],
  ...
  // ─── Файлы ────────────────────────────────────────────────────────────────
  {
    ...
    // Английские имена папок — это их СОБСТВЕННЫЕ имена в Windows: человек
    // читает «Downloads» в проводнике и так же говорит. Прямой слэш тоже
    // принимаем — его подставляют вместо обратного и голосом, и копипастой.
    patterns: [new RegExp(
      `^(?:открой|открыть|покажи|open|show)\\s+(?:папку\\s+|folder\\s+)?(?<path>загрузки|документы|рабочий стол|картинки|изображения|музыка|видео|` +
      `downloads?|documents|docs|desktop|pictures|images|music|videos?|home|[a-z]:[\\\\/].*)$`, 'i')],
    examples: ['открой загрузки', 'открой папку C:\\Projects', 'открой downloads'],
    // Осторожно с примерами имён папок: слово, встретившееся в двух фразах,
    // считается служебным и вырезается из аргумента — тогда «открой папку
    // Загрузки» доходит до действия с пустым путём.
    phrases: ['покажи в проводнике', 'открой каталог'],
  ...
  {
    id: 'list_files',
    title: 'Показать файлы в папке',
    description: 'Показывает содержимое папки (знает «загрузки», «документы»… или полный путь)',
    category: 'files',
    // read_dir/ls/dir — как модель чаще всего называет это действие сама; без
    // псевдонимов её вызов падал с «Неизвестное действие: read_dir»
    aliases: ['read_dir', 'list_dir', 'ls', 'dir', 'что в папке', 'содержимое папки'],
    patterns: [/^(?:что|покажи(?:\s+что)?)\s+в\s+(?:папке\s+)?(?<path>загрузки|документы|рабочем столе|рабочий стол|картинки|музыка|видео|[a-z]:\\.+)$/],
    examples: ['что в папке загрузки', 'покажи что в документах'],
    phrases: ['покажи файлы в папке', 'список файлов', 'что лежит в загрузках'],
    args: [{ name: 'path', description: 'Имя известной папки или путь', required: true }],
    softFail: true,
    execute: (a) => FileController.listDir(a.path ?? '')
  },
  {
    id: 'create_folder',
    title: 'Создать папку',
  ...
  },
  {
    ...
    category: 'files',
    aliases: ['удалить'],
    // Файловые команды были доступны ТОЛЬКО нейросети: у действия не было ни
    // одного шаблона, и «удали файл temp.log» уходило в облако вместе с именем
    // файла. Теперь их выполняет ядро — мгновенно и без интернета, а
    // подтверждение никуда не делось: действие остаётся опасным.
    patterns: [/^(?:удали|удалить|убери)\s+(?:файл|папку)\s+(?<path>.+)$/],
    examples: ['удалить файл', 'удали файл temp.log'],
  ...
  },
  {
    ...
    patterns: [/^(?:переименуй|переименовать)\s+(?:файл\s+|папку\s+)?(?<path>.+?)\s+в\s+(?<name>[^\s]+)$/],
    examples: ['переименуй файл a.txt в b.txt'],
    args: [
  ...
  },
  {
    ...
    category: 'files',
    aliases: ['копировать'],
    patterns: [/^(?:скопируй|копируй|скопировать)\s+(?:файл\s+|папку\s+)?(?<from>.+?)\s+(?:в|на)\s+(?<to>.+)$/],
    examples: ['скопируй report.pdf в C:\\Архив'],
    args: [
  ...
  },
  {
    ...
    category: 'files',
    aliases: ['переместить'],
    patterns: [/^(?:перемести|переместить|перенеси)\s+(?:файл\s+|папку\s+)?(?<from>.+?)\s+(?:в|на)\s+(?<to>.+)$/],
    examples: ['перемести отчёт.docx в C:\\Архив'],
    args: [
      { name: 'from', description: 'Откуда', required: true },
      { name: 'to', description: 'Куда', required: true }
    ],
  ...
  },
  {
    ...
    patterns: [
      /^(?:загрузка (?:системы|компьютера|пк)|(?:нагрузка|состояние) системы|сколько (?:оперативки|памяти|озу)(?:\s+(?:занято|свободно))?|нагрузка на процессор)$/,
      /^как(?:ая|ой)?\s+(?:сейчас\s+)?(?:загрузка|нагрузка)(?:\s+(?:на\s+)?(?:процессор(?:а|е)?|цп|системы|компьютера|пк|памяти))?$/,
      /^(?:как|насколько)\s+загружен\s+(?:компьютер|комп|пк|процессор)$/
    ],
  ...
  // ─── OCR: текст с экрана ──────────────────────────────────────────────────
  {
    ...
    patterns: [
      /^(?:прочитай|распознай|считай|извлеки)\s+текст(?:\s+(?:с экрана|на экране|со скриншота|с картинки))?$/,
      /^(?:прочитай|распознай|посмотри на|глянь на)\s+экран$/,
      /^(?:скажи|расскажи|покажи)\s+(?:мне\s+)?что\s+(?:там\s+)?на\s+экране$/,
      /^текст на экране$/
    ],
  ...
  },
  {
    ...
    category: 'apps',
    aliases: ['запусти', 'открой', 'open', 'launch'],
    // До четырёх слов: «Visual Studio Code», «Adobe Photoshop 2024»,
    // «Sublime Text 3» — обычные названия программ, а прежний потолок в два
    // слова отправлял их в облако. Ловушка и так проверяется последней, а имя
    // из служебных слов отсекает APP_STOPWORDS.
    patterns: [new RegExp(`^(?:${OPEN_VERBS})\\s+(?<app>[\\wа-я][\\w.а-я+&-]*(?:\\s+[\\w.а-я+&-]+){0,3})$`, 'i')],
    // УНИВЕРСАЛЬНАЯ ЛОВУШКА: «открой что угодно» подходит почти под любую фразу,
    // поэтому проверяется последней — иначе перехватила бы «открой браузер»,
    // «включи музыку» и прочие конкретные команды.
    priority: -1,
  ...
  },
  {
    ...
    aliases: ['файл', 'file'],
    patterns: [
      new RegExp(`^(?:${OPEN_VERBS})\\s+файл\\s+(?<path>.+)$`),
      // Слово «файл» больше не обязательно. Человек говорит «открой
      // report.pdf» или «открой D:\\Games\\readme.md» — и до сих пор это уходило
      // в облако: путь с обратными слэшами не подходил ни под один шаблон, а
      // имя с расширением перехватывал «открыть сайт».
      //
      // Расширение обязательно в ОБОИХ шаблонах: без него «открой C:\\Projects»
      // (папка) и «открой youtube.com» (сайт) достались бы этому действию.
      new RegExp(`^(?:${OPEN_VERBS})\\s+(?<path>(?:[a-z]:[\\\\/]|\\\\\\\\)[^\\n]*\\.(?:${FILE_EXT}))$`, 'i'),
      new RegExp(`^(?:${OPEN_VERBS})\\s+(?<path>[^\\\\/:*?"<>|\\n]+\\.(?:${FILE_EXT}))$`, 'i')
    ],
    // Проверяется РАНЬШЕ «открыть папку»: путь с расширением — это файл, и
    // открывать его проводником вместо нужной программы неправильно.
    priority: 1,
    examples: ['открой файл C:\\doc.pdf', 'открой report.pdf', 'открой D:\\Games\\readme.md'],
    phrases: ['открой документ', 'покажи файл', 'запусти файл'],
    args: [{ name: 'path', description: 'Путь к файлу', required: true }],
    softFail: true,
    execute: (a) => FileController.openFile(a.path ?? ''),
    confirmText: () => 'Открываю'
  },
  // ─── Счёт, перевод, заметки, напоминания, поиск файлов ────────────────────
  /*
   * Всё ниже ядро уже УМЕЛО в своих модулях — но наружу выведено не было, и
   * каждая такая просьба уходила в облако вместе с содержимым: именем файла,
   * текстом заметки, тем, о чём человек просит напомнить. Теперь это обычные
   * локальные команды: мгновенно, офлайн и никуда не отправляется.
   */
  {
    id: 'calculate',
    title: 'Посчитать',
    description: 'Считает выражение: сложение, вычитание, умножение, деление, проценты, степень, корень',
    category: 'info',
    aliases: ['посчитай', 'вычисли', 'калькулятор'],
    patterns: [
      /^(?:сколько будет|посчитай|вычисли|сосчитай|реши)\s+(?<expr>.+)$/,
      // голое выражение знаками — «2+2*3»: букв в нём нет, спутать не с чем
      /^(?<expr>[\d\s+\-*/^().,%]+)$/
    ],
    examples: ['сколько будет 15 умножить на 7', 'посчитай 20% от 350'],
    phrases: ['посчитай пожалуйста', 'помоги посчитать'],
    args: [{ name: 'expr', description: 'Выражение', required: true }],
    softFail: true,
    // «сколько будет» бывает и не про счёт («сколько будет стоить») — считаем
    // только то, где действительно есть числа и знак действия
    validate: (a) => (UtilityController.looksLikeMath(a.expr ?? '') ? null : 'это не выражение'),
    execute: async (a) => UtilityController.calculate(a.expr ?? '')
  },
  {
    id: 'translate',
    title: 'Перевести',
    description: 'Переводит текст на другой язык',
    category: 'info',
    aliases: ['переведи', 'перевод', 'translate'],
    patterns: [
      /^переведи\s+(?:на\s+(?<lang>[а-яa-z]+)\s+)?(?<text>.+?)(?:\s+на\s+(?<lang2>[а-яa-z]+))?$/
    ],
    examples: ['переведи hello на русский', 'переведи на английский добрый день'],
    args: [
      { name: 'text', description: 'Что перевести', required: true },
      { name: 'lang', description: 'На какой язык' },
      { name: 'lang2', description: 'На какой язык' }
    ],
    softFail: true,
    execute: async (a) => UtilityController.translate(a.text ?? '', a.lang || a.lang2)
  },
  {
    id: 'note_add',
    title: 'Записать заметку',
    description: 'Сохраняет заметку в память Kira',
    category: 'info',
    aliases: ['заметка', 'запиши'],
    patterns: [/^(?:запиши|записать|заметь|добавь)\s+(?:в\s+заметки|заметку)\s+(?<text>.+)$/],
    examples: ['запиши в заметки купить хлеб'],
    args: [{ name: 'text', description: 'Текст заметки', required: true }],
    execute: async (a) => NoteController.add(a.text ?? ''),
    confirmText: () => 'Записала'
  },
  {
    id: 'notes_list',
    title: 'Мои заметки',
    description: 'Показывает сохранённые заметки',
    category: 'info',
    aliases: ['заметки', 'мои заметки'],
    patterns: [/^(?:покажи\s+)?(?:мои\s+)?заметки$|^что\s+(?:я\s+)?(?:записал|записывал)(?:а)?$/],
    examples: ['покажи мои заметки'],
    phrases: ['что у меня в заметках', 'список заметок'],
    args: [],
    execute: async () => NoteController.list()
  },
  {
    id: 'remind',
    title: 'Напомнить',
    description: 'Ставит напоминание на время («через час», «завтра в 9», «каждый день в 9»)',
    category: 'info',
    aliases: ['напомни', 'напоминание', 'remind'],
    // время может стоять и до, и после самой просьбы — люди говорят и так, и так
    /*
     * Время может стоять и до просьбы, и после — люди говорят и так, и так.
     * Само указание времени описано ЦЕЛИКОМ: «завтра в 9» это одно указание, а
     * не «завтра» плюс невнятный хвост. Иначе разбор откусывал только «завтра»,
     * и напоминание вставало на девять утра по умолчанию вместо названного часа.
     */
    patterns: [
      /^напомни(?:\s+мне)?\s+(?<when>(?:через\s+(?:\d+\s*)?\S+|(?:после)?завтра|сегодня|вечером|утром|днем|ночью|кажд\S+(?:\s+\S+)?|в\s+\d{1,2}(?::\d{2})?)(?:\s+(?:утром|вечером|днем|ночью))?(?:\s+в\s+\d{1,2}(?::\d{2})?)?)\s+(?<text>.+)$/,
      /^напомни(?:\s+мне)?\s+(?<text2>.+?)\s+(?<when2>через\s+(?:\d+\s*)?\S+.*|(?:после)?завтра.*|сегодня.*|кажд\S+.*|в\s+\d{1,2}(?::\d{2})?)$/
    ],
    examples: ['напомни через час позвонить маме', 'напомни завтра в 9 про встречу'],
    args: [
      { name: 'when', description: 'Когда' },
      { name: 'text', description: 'О чём' },
      { name: 'when2', description: 'Когда' },
      { name: 'text2', description: 'О чём' }
    ],
    softFail: true,
    execute: async (a) => ReminderController.add(a.text || a.text2 || '', a.when || a.when2 || ''),
    confirmText: () => 'Напомню'
  },
  {
    id: 'reminders_list',
    title: 'Мои напоминания',
    description: 'Показывает запланированные напоминания',
    category: 'info',
    aliases: ['напоминания'],
    patterns: [/^(?:покажи\s+)?(?:мои\s+)?напоминания$|^что\s+(?:мне\s+)?напомнить$/],
    examples: ['покажи напоминания'],
    phrases: ['список напоминаний', 'о чём я просил напомнить'],
    args: [],
    execute: async () => ReminderController.list()
  },
  {
    id: 'find_file',
    title: 'Найти файл',
    description: 'Ищет файл по названию в папках пользователя',
    category: 'files',
    aliases: ['найди файл', 'поиск файла'],
    patterns: [/^(?:найди|поищи|где)\s+(?:файл|папку|документ)\s+(?<query>.+)$/],
    examples: ['найди файл отчет'],
    phrases: ['найди мой файл', 'поищи документ'],
    args: [{ name: 'query', description: 'Часть названия', required: true }],
    softFail: true,
    execute: async (a) => FileController.find(a.query ?? ''),
    confirmText: (a) => `Ищу: ${a.query}`
  },
  {
    id: 'read_file',
    title: 'Прочитать файл',
    description: 'Показывает содержимое файла или документа (PDF, Word, Excel)',
    category: 'files',
    aliases: ['прочитай файл'],
    patterns: [/^(?:прочитай|покажи содержимое|открой и прочитай)\s+(?:файл\s+|документ\s+)?(?<path>[^\n]+\.[a-z0-9]{1,6})$/i],
    examples: ['прочитай файл C:\\заметки.txt'],
    args: [{ name: 'path', description: 'Путь к файлу', required: true }],
    softFail: true,
    execute: async (a) => FileController.readFile(a.path ?? '')
  },
  {
    id: 'create_file',
    title: 'Создать файл',
    description: 'Создаёт текстовый файл на рабочем столе (или по указанному пути)',
    category: 'files',
    aliases: ['создай файл'],
    patterns: [/^(?:создай|сделай|создать)\s+файл\s+(?<name>[^\n]+?)(?:\s+с\s+текстом\s+(?<text>.+))?$/],
    examples: ['создай файл заметки.txt', 'создай файл список.txt с текстом хлеб, молоко'],
    args: [
      { name: 'name', description: 'Имя или путь файла', required: true },
      { name: 'text', description: 'Содержимое' }
    ],
    softFail: true,
    execute: async (a) => {
      const raw = (a.name ?? '').trim()
      // без пути кладём на рабочий стол: это то место, где человек его найдёт
      const path = /^[a-z]:[\\/]|^\\\\/i.test(raw) ? raw : FileController.desktopPath(raw)
      return FileController.writeFile(path, a.text ?? '')
    },
    confirmText: (a) => `Создала ${a.name}`
  },
  {
    id: 'set_wallpaper',
    title: 'Сменить обои',
    description: 'Ставит картинку обоями рабочего стола',
    category: 'system',
    aliases: ['обои', 'wallpaper'],
    patterns: [/^(?:смени|поставь|установи|сделай)\s+обои(?:\s+(?:на\s+)?(?<path>.+))?$/],
    examples: ['поставь обои C:\\Users\\Public\\Pictures\\закат.jpg'],
    args: [{ name: 'path', description: 'Путь к картинке', required: true }],
    softFail: true,
    execute: async (a) => {
      // Уточнение делает САМО действие, а не validate: неудачная проверка
      // молча уводит запрос в облако (см. engine), и вопрос человек бы не
      // услышал. А спросить нужно — угадывать картинку за него неправильно.
      const path = (a.path ?? '').trim()
      if (!path) return { ok: true, message: 'Скажи, какую картинку поставить обоями — назови файл или путь к нему' }
      return SystemController.setWallpaper(path)
    },
    undo: () => SystemController.restoreWallpaper(),
    confirmText: () => 'Обои сменила'
  },
  {
    id: 'undo_last',
