...
export interface KiraAction {
  ...
  patterns?: RegExp[]
  /**
   * Приоритет сопоставления: чем больше, тем раньше проверяется действие.
   *
   * Раньше приоритет определялся ПОРЯДКОМ в массиве — вставил действие не в то
   * место, и поведение менялось молча (например, универсальный «запусти что-то»
   * перехватывал «запусти браузер»). Теперь порядок в файле — вопрос
   * читаемости, а приоритет считается сам: чем больше в шаблоне буквального
   * текста и меньше «любого текста», тем он специфичнее и выше.
   *
   * Указывать вручную стоит редко — только когда автоматическая оценка
   * ошибается. Универсальные «ловушки» помечают отрицательным значением.
   */
  priority?: number
  examples: string[]
  /**
