...
function clean(raw: string): string {
  return raw
    /*
     * Служебная пометка в начале — не часть просьбы.
     *
     * Голосовой режим при включённом «чувстве эмоций» приписывает спереди
     * «[голос звучит устало] », и все шаблоны ядра анкерные — они начинаются с
     * ^. Одна такая пометка убивала распознавание ЦЕЛИКОМ: «открой браузер»
     * переставало быть командой, и каждая фраза уезжала в облако. Заметить это
     * тем труднее, что при выключенной настройке всё работает.
     */
    .replace(/^\s*\[[^\]\n]{0,60}\]\s*/, '')
    .replace(/^\s*кира[,!.\s]+/i, '')
